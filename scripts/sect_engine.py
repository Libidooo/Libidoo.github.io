...
# 主体优先级（数值越小优先级越高）
# 排序依据：宗派信号可信度（宗派专属性强度）
# 弥勒最强（与唯识宗教义祖师关系）→ 阿弥陀（净土宗信仰核心）→
# 观音（已有子分类机制）→ 地藏（无专属宗派）→ 药师（宗派专属性最弱）
SUBJECT_PRIORITY = {
    "10": 1,  # 弥勒（信号可信度最强：与唯识宗教义祖师关系）
    "3": 2,   # 阿弥陀（净土宗信仰核心，造像规模最大）
    "2": 3,   # 观音（已有子分类机制）
    "9": 4,   # 地藏（无专属宗派）
    "4": 5,   # 药师（宗派专属性最弱：跨净土/密教/天台）
    "5": 10,  # 密教（W≥60时通过显式逻辑优先，不走优先级）
    "6": 20,  # 华严
    "7": 30,  # 禅宗
    "11": 40, # 三论
    "12": 50, # 唯识
...
class MutexResolver:
    ...
    def resolve(
        ...
        消解规则:
        1. 密教强因（W≥60）优先：密教为主判卷，其他卷作为协同卷
           （密教与其他卷不再是⛔互斥，改为⚠可共存，但密教W≥60
           时作为主判卷保留优先权）
        2. 主体卷优先级：弥勒(10) > 阿弥陀(3) > 观音(2) > 地藏(9) > 药师(4)
           依据：宗派信号可信度（宗派专属性强度）
        3. ⛔ 互斥：保留 S 得分较高的卷
        4. ⚠ 协同：两卷并列标注为混合判定
        ...
        # 密教强因优先：检查是否有W≥60的密教词条
        esoteric_strong = False
        if "5" in matches_by_vid:
            for m in matches_by_vid["5"]:
                if m["w"] is not None and m["w"] >= ESOTERIC_THRESHOLD:
                    esoteric_strong = True
                    break

        if esoteric_strong:
            # 密教W≥60 → 密教为主判卷，其他卷作为协同卷
            top_vid = "5"
            conflicts = []
        else:
        ...
        cooperative_vids = []
        if esoteric_strong:
            # 密教主判时，所有其他有得分的卷都是协同卷（密教不再互斥）
            for vid in scores:
                if vid == top_vid:
                    continue
                if scores.get(vid, 0) >= 40:
...
def main():
    ...
    if args.test:
        ...
        test_cases = [
            ...
            ("纯文本: 毗沙门天王", "敬造毗沙门天王一躯"),
            ("纯文本: 三阶教", "三阶普法普敬无尽藏末法泥龛认恶"),
            ("纯文本: 密教与药师共存", "造药师琉璃光如来曼荼罗真言灌顶陀罗尼"),
            ("纯文本: 无匹配", "今日天气晴朗"),
        ]
